package com.abc.us.accounting.iface.domain.entity.oms

import com.abc.us.accounting.iface.domain.type.oms.IfChargeItemType
import io.hypersistence.utils.hibernate.type.json.JsonType
import java.math.BigDecimal
import java.time.OffsetDateTime
import jakarta.persistence.*
import org.hibernate.annotations.Comment
import org.hibernate.annotations.Type

/**
 * 서비스 청구 항목 정보 테이블 (HISTORY)
 */
//@Entity
@Table(name = "if_service_charge_item")
@Comment("서비스 청구 항목(HISTORY)")
class IfServiceChargeItem(

    @Comment("ID")
    @Id
    @Column(name = "id", nullable = false)
    val id: String,

    @Comment("서비스 청구항목 ID")
    @Column(name = "charge_item_id", nullable = false)
    val chargeItemId: String,

    @Comment("청구아이템 유형")
    @Column(name = "charge_item_type", nullable = false)
    @Enumerated(EnumType.STRING)
    val chargeItemType: IfChargeItemType,

    @Comment("수량")
    @Column(name = "quantity", nullable = false)
    val quantity: String,

    @Comment("전체금액(세금제외)")
    @Column(name = "subtotal_price", nullable = false, precision = 38, scale = 4)
    val subtotalPrice: BigDecimal,

    @Comment("아이템금액")
    @Column(name = "item_price", nullable = false, precision = 38, scale = 4)
    val itemPrice: BigDecimal,

    @Comment("아이템 할인금액")
    @Column(name = "discount_price", nullable = false, precision = 38, scale = 4)
    val discountPrice: BigDecimal,

    @Comment("선지급 금액")
    @Column(name = "prepaid_amount", nullable = false, precision = 38, scale = 4)
    val prepaidAmount: BigDecimal,

    @Comment("프로모션 정보")
    @Type(JsonType::class)
    @Column(name = "promotions", columnDefinition = "json")
    val promotions: MutableMap<String,Any> = mutableMapOf(),

    @Comment("통화")
    @Column(name = "currency", nullable = false)
    val currency: String,

    @Comment("면세여부")
    @Column(name = "is_tax_exempt", nullable = false)
    val isTaxExempt: Boolean,

    @Comment("서비스청구 ID")
    @Column(name = "service_charge_id", nullable = false)
    val serviceChargeId: String,

    @Comment("자재ID")
    @Column(name = "material_id")
    val materialId: String? = null,

    @Comment("생성시간")
    @Column(name = "create_time", nullable = false)
    val createTime: OffsetDateTime,

    @Comment("수정시간")
    @Column(name = "update_time", nullable = false)
    val updateTime: OffsetDateTime
)
