...
class OleaseService(
    ...
    fun findRentalAssetDepreciationScheduleBaseData(
        ...
        val installation = collectInstallationRepository.findTopOrderByInstallationTime(
            serialNumber
        ) ?: return null
        val contract = collectContractRepository.findTopOrderByCreateTime(
            installation.orderItemId!!,
            listOf(ContractStatus.ACTIVE.name)
        ) ?: return null
        val materials = collectMaterialRepository.findAllByMaterialIdIn(
            listOf(contract.materialId!!)
        )
        if (materials.isEmpty()) {
            return null
        }
        val material = materials.first()
        val orders = collectOrderRepository.findActiveByOrderIdsIn(
            listOf(contract.orderId!!)
        )
        ...
        val res = RentalAssetDepreciationScheduleBaseData(
            ...
            orderDate = order.orderCreateTime?.toLocalDate(),
            installationDate = installation.installationTime?.toLocalDate(),
    ...
    fun deleteAllHistory() {
        rentalAssetHistoryRepository.deleteAll()
    }
}
