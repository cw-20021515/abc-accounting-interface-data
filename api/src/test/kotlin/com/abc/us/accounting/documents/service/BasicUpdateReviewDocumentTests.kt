...
@ActiveProfiles("test")
class BasicUpdateReviewDocumentTests  (
    ...
    @Test
    fun `basic update review documents success`() {
        val context = DocumentServiceContext.SAVE_DEBUG
        val documentCount = 3

        // 1) 임시전표 생성
        logger.info("before document generate: ${documentCount}")
        val posted = eachTest(context, 1, count = documentCount, requestType = SUBMIT, companyCode = companyCode)
        logger.info("after document posted : ${posted.size}")

        // 2) 정식전표 생성 (전표 항목 숫자가 다른 것으로)
        val results = documentService.updateReviewStatus(context, posted.map { it.docId }, POSTING)

        results.size shouldBe documentCount

        posted.zip(results).forEach { (request, result) ->
            request.docId shouldBe result.docId
            request.docType shouldBe result.docType
            request.docItems.size shouldBe result.docItems.size
        }
...
//        val templateMapping = DocumentTemplateMapping.findByTemplateCode(docOrigin.docTemplateCode)
    ...
    fun eachTest(context:DocumentServiceContext, iteration:Int, count:Int=1, requestType: RequestType, docType:DocumentType?=null, companyCode: CompanyCode? = null):List<DocumentResult> {
        ...
        val results = when(requestType) {
            ...
            SUBMIT -> documentService.submit(context, requests)
            else -> emptyList()
        }
