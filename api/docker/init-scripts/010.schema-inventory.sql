...
CREATE INDEX "ix_inventory_costing_costing_date_warehouse_id_material_id" ON "inventory_costing" ("costing_date" DESC, "warehouse_id", "material_id");
COMMENT ON TABLE "inventory_costing" IS '자재별 재고 원가 계산';
COMMENT ON COLUMN "inventory_costing"."id" IS '재고원가계산 ID';
COMMENT ON COLUMN "inventory_costing"."company_code" IS '재고 보유 회사 코드';
COMMENT ON COLUMN "inventory_costing"."costing_date" IS '원가 계산일';
COMMENT ON COLUMN "inventory_costing"."method" IS '재고 원가 결정 방법(InventoryCostingMethod)';
...
(
    ...
    "is_active" char NOT NULL CONSTRAINT inventory_valuation_is_active  check (is_active = ANY (ARRAY ['N'::bpchar, 'Y'::bpchar])),
    "create_time" timestamp(6) with time zone DEFAULT (now())
    );
CREATE INDEX "ix_inventory_valuation_closing_date" ON "inventory_valuation" ("closing_date" DESC);
COMMENT ON TABLE "inventory_valuation" IS '기말 자재별 전체 재고자산가치평가';
COMMENT ON COLUMN "inventory_valuation"."id" IS '재고자산평가 ID';
COMMENT ON COLUMN "inventory_valuation"."company_code" IS '재고 보유 회사 코드';
COMMENT ON COLUMN "inventory_valuation"."costing_id" IS '재고원가계산 ID';
COMMENT ON COLUMN "inventory_valuation"."closing_date" IS '기말 평가일';
